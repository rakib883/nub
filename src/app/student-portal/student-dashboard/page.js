...
const StudentProfile = () => {
  ...
  // 1. Local Storage theke email neya
  // Window check kora hoyeche jate error na dey
  const getEmail = () => {
    if (typeof window !== "undefined") {
      const myData = JSON.parse(localStorage.getItem("student"));
      return myData?.email || "";
    }
    return "";
  };

  const singleUserEmail = getEmail();

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://nub-bakend.vercel.app/api/all-student');
        const result = await response.json();
        // Backend jodi { data: [...] } format-e pathay, tobe setStudents(result.data) hobe
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black">L O A D I N G . . .</div>;

  return (
    <div className="min-h-screen bg-[#f8fafc] py-10 px-4">
      <div className="max-w-4xl mx-auto">
        
        {/* 2. Filter ebong Map Logic */}
        {students
          .filter(student => student.email === singleUserEmail) // Sudhu matching email ta rakhbe
          .map((studentData) => (
            <div key={studentData._id || studentData.id}>
              ...
              <div className="bg-white rounded-t-[2rem] shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-center gap-6">
                <div className="relative">
                  <div className="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-5xl font-bold border-4 border-white shadow-md uppercase">
                ...
                <div className="text-center md:text-left flex-1">
                  ...
                  <p className="text-gray-500 font-medium flex items-center justify-center md:justify-start gap-2">
                    <FaIdCard className="text-blue-500" /> ID: {studentData._id?.slice(-8).toUpperCase() || "N/A"}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                  ...
                  <div className="bg-[#011e40] p-8 rounded-[2rem] shadow-xl text-white relative overflow-hidden">
                     <div className="relative z-10">
                        ...
                        <div className="w-full bg-blue-900/50 rounded-full h-3 mb-2">
                          <div className="bg-[#f1c40f] h-3 rounded-full w-[100%] shadow-[0_0_10px_#f1c40f]"></div>
                        </div>
                        <p className="text-sm font-bold text-blue-200">Payment Status: {studentData.dueAmount === 0 ? "Fully Paid" : "Partial Paid"}</p>
                     </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 h-full">
                    <h3 className="text-lg font-black text-[#011e40] mb-6 flex items-center gap-2 border-b pb-3">
                      <FaMoneyBillWave className="text-green-500" /> Billing Summary
                    </h3>
                    ...
                    <div className="space-y-5">
                      ...
                      <button className="w-full bg-[#011e40] text-[#f1c40f] py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg hover:opacity-90 transition-all">
                        Pay Now
                      </button>
                    </div>
                  </div>
        ...
        {/* User jodi na pawa jay */}
        {!loading && students.filter(s => s.email === singleUserEmail).length === 0 && (
          <div className="text-center p-20 bg-white rounded-[2rem] border border-dashed border-gray-300">
             <h2 className="text-2xl font-black text-gray-400">Profile Not Found in Database</h2>
          </div>
        )}
...
const InfoItem = ({ icon, label, value }) => (
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <p className="text-sm font-bold text-[#011e40] break-words uppercase">{value}</p>
    </div>
  </div>
);
